...
/*
  @Fuction: SHA1
  @Param: uint8_t* sha1: output, 160bit(20bytes), calculated sha1 value
		  uint8_t* input: will be SHA1 data, must have 20byte
		  int size: assign how many byte of input
...
uint8_t SHA1(uint8_t* sha1, uint8_t* input, int size)
{
	...
	if (input && sha1 && size > 0)
	{
		...
		if (padNum != 0)
		{
			if (padNum <= 56)
			{
				data = (uint8_t*)malloc(size + 64 - padNum);
				if (data == nullptr)	
					return 0;
				memcpy(data, input, size);
				memcpy(data + size, padding, 56 - padNum);
				data[size + 56 - padNum] = (uint8_t)(((int64_t)size * 8) >> 56);
				data[size + 57 - padNum] = (uint8_t)(((int64_t)size * 8) >> 48);
				data[size + 58 - padNum] = (uint8_t)(((int64_t)size * 8) >> 40);
				data[size + 59 - padNum] = (uint8_t)(((int64_t)size * 8) >> 32);
				data[size + 60 - padNum] = (uint8_t)(((int64_t)size * 8) >> 24);
				data[size + 61 - padNum] = (uint8_t)(((int64_t)size * 8) >> 16);
				data[size + 62 - padNum] = (uint8_t)(((int64_t)size * 8) >> 8);
				data[size + 63 - padNum] = (uint8_t)(((int64_t)size * 8));
				blockNum++;
			}
			else {
				data = (uint8_t*)malloc(size + 128 - padNum);
				if (data == nullptr)	
					return 0;
				memcpy(data, input, size);
				memcpy(data + size, padding, 120 - padNum);
				data[size + 120 - padNum] = (uint8_t)(((int64_t)size * 8) >> 56);
				data[size + 121 - padNum] = (uint8_t)(((int64_t)size * 8) >> 48);
				data[size + 122 - padNum] = (uint8_t)(((int64_t)size * 8) >> 40);
				data[size + 123 - padNum] = (uint8_t)(((int64_t)size * 8) >> 32);
				data[size + 124 - padNum] = (uint8_t)(((int64_t)size * 8) >> 24);
				data[size + 125 - padNum] = (uint8_t)(((int64_t)size * 8) >> 16);
				data[size + 126 - padNum] = (uint8_t)(((int64_t)size * 8) >> 8);
				data[size + 127 - padNum] = (uint8_t)(((int64_t)size * 8));
				blockNum += 2;
			}
		}
		else {
